...
export function DateRangeDropdown(props: DateRangeDropdownProps) {
    ...
    const [startDate, setStartDate] = useState<Date | undefined>(props?.dateRange?.start);
    const [endDate, setEndDate] = useState<Date | undefined>(props?.dateRange?.end);
    ...
    const handleDropdownChange = (_e: React.FormEvent<HTMLDivElement>, option?: IDropdownOption) => {
        ...
        props.onDateValueChange({ start: start as Date, end }, option.key as DateDropdownValues, labels[option.key as DateDropdownValues]);
    };

    const handleStartDateChange: IDatePickerProps['onSelectDate'] = (date) => {
        let start = new Date(date as Date);
        start.setHours(0, 0, 0, 0);
